...
<div on:click on:click={() => viewable ? img_path.set(img) : ''} class="relative overflow-hidden {viewable ? 'cursor-pointer' : ''} {clazz}" {style}>
  {#if !loaded}
    <SkeletonComponent />
  {/if}
  {#if !element}
    <div class="absolute top-0 left-0 bg-cover bg-center w-full h-full" bind:this={i_ref} />
  {:else}
