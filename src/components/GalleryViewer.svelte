...
{#if show}
  <div
    transition:fade
    class="fixed w-screen h-screen bg-black bg-opacity-75 z-9998"
    on:click={() => {
      img_path.set('')
    }} />
  <div
    transition:slide
    class="grid fixed w-screen h-screen justify-center items-center pointer-events-none z-9998 p-4"
    on:click={() => {
      img_path.set('')
    }}>
