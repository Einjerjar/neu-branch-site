<script>
  ...
  const getBranches = async () => {
    const response = await fetch(
      re_param('collections/get/branch_data', {
        'fields[id]': true,
        'fields[name]': true,
...
<div transition:slide>
  <div class="w-full h-15 bg-primary-900 text-xl text-white uppercase font-semibold py-8 flex items-center justify-center md:(text-2xl h-18) lg:(text-4xl h-25)" in:fade={{ duration: 600 }}>university branches</div>
  ...
  <div class="bg-gray-200 pb-16" in:fade={{ duration: 600 }}>
    <div class="container mx-auto shadow-lg bg-white rounded py-5">
      <div class="mx-auto items-center bg-white p-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {:then branches}
          {#each branches as branch}
            <Branch branchImage="{HOST_ROOT}{branch.cover_image?.path || ''}" branchName={branch.name} />
          {/each}
        {:catch}
      ...
      <div class="text-primary-900 text-2xl sm:text-3xl md:text-4xl font-bold border-b-6 p-8 mt-4 border-primary-900">School Facilities</div>
          ...
          <div class="p-8 grid grid-cols-2 gap-4 items-center">
            ...
            <div class="md:w-2/4">
              <div class="p-2 shadow rounded-md">
                <SkeletonImage class="w-full h-auto" img="{HOST_ROOT}{facility.value.image?.path || ''}" element={true} />
              </div>
            </div>
          </div>
        {/each}
